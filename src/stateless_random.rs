...
pub fn random_float<T: Float>(min: T, max: T, index: usize, seed: u64) -> T {
    let random_uint = random_uint::<T>(index, seed);
    T::from(random_uint).unwrap() / T::from(u32::MAX).unwrap() * (max - min) + min
}

pub fn random_float_default<T: Float>(index: usize, seed: u64) -> T {
    random_float(T::zero(), T::one(), index, seed)
}

pub fn random_vector2<T: Float>(index: usize, seed: u64) -> Vector2<T> {
    Vector2::<T>::new(
...
pub fn random_vector3<T: Float>(index: usize, seed: u64) -> Vector3<T> {
    Vector3::<T>::new(
...
pub fn random_vector4<T: Float>(index: usize, seed: u64) -> Vector4<T> {
    Vector4::<T>::new(
